...
####################################################################
binpath="/home/quentin/scratch/DILS_4pop/bin" # binpath: path to binaries for ABC inferences
...
rule simulations_opti1:
	...
	output:
		...
		#expand("{datapath}/opti1/best_{wildcards.iteration}/ABCstat.txt", datapath=datapath),
		#expand("{datapath}/opti1/best_{wildcards.iteration}/priorfile.txt", datapath=datapath)
	shell:
		"bestTopo=$(cat {datapath}/modelComp/best_model_vector.txt | awk '{{print $1}}'); "
		"bestAB=$(cat {datapath}/modelComp/best_model_vector.txt | awk '{{print $2}}'); "
		"bestAC=$(cat {datapath}/modelComp/best_model_vector.txt | awk '{{print $3}}'); "
